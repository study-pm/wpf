...
- [Events / События](#events--события)
  - [Events in XAML / События в XAML](#events-in-xaml--события-в-xaml)
    - [Подписка на событие из отделенного кода](#подписка-на-событие-из-отделенного-кода)
...
### Events in XAML / События в XAML
Большинство современных UI-фреймворков основаны на событиях (являются event driven), и WPF здесь не является исключением. Все элементы управления, включая `Window` (который также наследует класс `Control`) публикуют ряд событий (expose events), на которые можно подписываться. Оформление подписки даёт возможность уведомлять приложение о наступлении событий, для того чтобы соответствующим образом реагировать на них.

Существует множество типов событий, но наиболее часто используются те, что дают возможность реагировать на взаимодействие пользователя с приложением посредством мыши или клавиатуры. У большинства элементов управления можно обнаружить такие события, как `KeyDown`, `KeyUp`, `MouseDown`, `MouseEnter`, `MouseLeave`, `MouseUp` и некоторые другие.

События в WPF являются непростой темой, и детально они будут рассмотрены чуть позже, однако для начала необходимо разобраться, как связать событие элемента управления в XAML с логикой в отделенном коде. Обратимся к следующему примеру:
```xml
...
Обратите внимание, как мы подписались на событие `MouseUp` элемента `Grid`, указав имя метода. Данный метод должен быть задан в отделенном коде в соответствии с определенной сигнатурой события. В данном случае он должен выглядеть следующим образом:
```cs
...
Событие `MouseUp` использует делегат под названием **`MouseButtonEventHandler`**, на который оформлена подписка. Он имеет два параметра — `sender` (*отправитель* — элемент, инициирующий событие — raise the event) и объект `MouseButtonEventArgs`, содержащий некоторую полезную информацию. В данном примере он используется для получения позиции указателя мыши и передачи этой информации пользователю.

Несколько событий могут пользоваться делегатами одного типа — например, события `MouseUp` и `MouseDown` используют делегат **`MouseButtonEventHandler`**, тогда как `MouseMove` — **`MouseEventHandler`**. При определении метода обработчика событий необходимо знать, какой делегат он использует. Если же это неизвестно, то всегда можно заглянуть в документацию.

К счастью, Visual Studio может помочь сгенерировать необходимый обработчик события. Самый простой способ сделать это — просто написать название события в XAML и позволить IntelliSense (системе автодополнения VS) автоматически сделать остальное.

При выборе **<New Event Handler>** Visual Studio сгенерирует подходящий обработчик событий в файле отделенного кода. Ему будет присвоено название в формате `<control name>_<event name>`, в нашем случае — **`pnlMainGrid_MouseDown`**. При нажатии правой кнопки мыши на названии события и выборе **Navigate to Event Handler** VS переместится прямо к обработчику в отделенном коде.

#### Подписка на событие из отделенного кода
Выше был продемонстрирован наиболее распространенный способ подписки на событие, но может возникнуть необходимость и подписаться на событие напрямую из отделенного кода. Это делается с помощью оператора C# `+=` , который позволяет добавить обработчик непосредственно к объекту, связанному с наблюдаемым событием. Ниже представлен пример того, как это сделать:
```cs
using System;
...
Опять же, здесь необходимо знать, какой именно делегат использовать, и снова Visual Studio может нас в этом выручить. Стоит только написать:
```cs
pnlMainGrid.MouseDown +=
```

как Visual Studio предложит помощь.

Нужно просто дважды нажать <kbd>Tab</kbd>, чтобы среда Visual Studio сгененировала необходимый обработчик событий прямо под текущим методом, уже готовый к имплементации. При оформлении подписки на события подобным образом необходимость что-то делать в XAML отпадает.
