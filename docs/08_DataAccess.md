...
Теперь, если вы запустите метод `DetectRelationshipChanges()`, то сможете убедиться, что изменения определяются автоматически с помощью прокси-объектов, еще до вызова метода `DetectChanges()`.
